...
double calcularGasto(int codMedico, double tarifaA, double tarifaB, double tarifaC, 
    ...
    validacion = (ultimasCifras >= 0 and ultimasCifras <= 34) or 
...
void asignarArreglos(int *arrDni, int *arrCantCitas, double *arrTiempo, 
    ...
    int posicion;
    double tiempoTotalHoras, tarifa;
    ...
    if(posicion >= 0){
        ...
        tarifa = calcularGasto(codMedico, tarifaA, tarifaB, tarifaC, 
                arrTiempo[posicion]);
        arrGasto[posicion] += tarifa*tiempoTotalHoras;
    }
    else{
        ...
        tarifa = calcularGasto(codMedico, tarifaA, tarifaB, tarifaC, 
                arrTiempo[posicion]);
        arrGasto[nPacientes] = tarifa * tiempoTotalHoras;
        nPacientes++;
    }
}
